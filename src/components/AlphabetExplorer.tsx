        ...
        <div>
            <div className="mb-5 flex flex-wrap gap-1.5">
                {TABS.map((t) => (
                    <button
                        key={t}
                        onClick={() => setTab(t)}
                        className={`rounded-lg px-3 py-1.5 text-sm font-medium transition ${tab === t ? "bg-accent text-white" : "border border-border hover:bg-accent/10"
                            }`}
                    >
                        {t}
                    </button>
            ...
            {tab === "Special" && (
                <>
                    ...
                    <CardBrowser cards={[...MODIFIERS, ...CONJUNCTS]} categories={["modifier", "conjunct"]} />
                </>
            )}
        </div>
    );
...
function Combinations() {
    ...
    return (
        <div>
            ...
            <div className="mb-4 flex flex-wrap gap-1.5">
                {CONSONANTS.map((c) => (
                    <button
                        ...
                        className={`telugu rounded-lg px-3 py-1 text-lg transition ${consonantId === c.id ? "bg-accent text-white" : "border border-border hover:bg-accent/10"
                            }`}
                    >
                        {c.telugu}
                    </button>
            ...
            <CardBrowser key={consonantId} cards={combos} categories={[]} />
        </div>
    );
}
